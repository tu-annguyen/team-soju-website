...
CREATE INDEX IF NOT EXISTS idx_pokedex_locations_region_name
  ON pokedex_locations(region, name);
...
CREATE INDEX IF NOT EXISTS idx_pokedex_encounters_horde
...
CREATE INDEX IF NOT EXISTS idx_hunt_spots_common ON hunt_spots(method, season, region, horde_size, time);
CREATE INDEX IF NOT EXISTS idx_hunt_spots_points ON hunt_spots(points_per_hour DESC, average_points DESC);
CREATE INDEX IF NOT EXISTS idx_hunt_spots_exp ON hunt_spots(exp_per_hour DESC);
CREATE INDEX IF NOT EXISTS idx_hunt_spot_species_name ON hunt_spot_species(name, spot_key);
CREATE INDEX IF NOT EXISTS idx_hunt_spot_species_family ON hunt_spot_species(family_key, spot_key);
CREATE INDEX IF NOT EXISTS idx_hunt_spot_species_tier_level ON hunt_spot_species(tier_number, min_level, spot_key);
CREATE INDEX IF NOT EXISTS idx_hunt_spot_egg_groups_group ON hunt_spot_egg_groups(egg_group, spot_key);
...
INSERT OR IGNORE INTO shiny_war_events (
  id, name, starts_at, ends_at, seasons_json, season_days
) VALUES (
  '2026',
  'PokeMMO Shiny Wars 2026',
...
CREATE INDEX IF NOT EXISTS idx_shiny_war_hunts_event_position
