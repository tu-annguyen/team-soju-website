...
type BoardDisplayMode = FeebasBoardDisplayMode;
...
const FeebasTileChecker = ({ apiBaseUrl, location, locale }: Props) => {
  ...
  useEffect(() => {
    const handleDisplayModeHotkey = (event: KeyboardEvent) => {
      ...
      if (!isFeebasDisplayModeHotkeyEvent(event) || isEditableHotkeyTarget(event.target)) {
        return;
      }

      if (normalizeFeebasDisplayModeHotkey(event.key) !== displayModeHotkey) {
        return;
      }
