...
function Sort({ filters: f, messages: m, setFilters }: SectionProps) {
  ...
  return <section aria-labelledby="hunt-sort-heading" className={section}>
    ...
    {f.sort === 'expPerHour' && <fieldset className="rounded-lg border p-4 sm:col-span-2 lg:col-span-4"><legend className="px-1 text-sm font-semibold">{m.charms}</legend><div className="flex flex-wrap gap-4">{([['', m.noExpCharm], ['0.25', m.expCharm25], ['0.5', m.expCharm50], ['1', m.expCharm100]] as const).map(([v, text]) => <label key={v || 'none'} className="flex items-center gap-2 text-sm"><input type="radio" name="exp-charm" checked={f.expCharm === v} onChange={() => set('expCharm', v)} />{text}</label>)}</div></fieldset>}
  </section>;
}
